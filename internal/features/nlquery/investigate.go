...
func (h *InvestigateHandler) RunScenario(w http.ResponseWriter, r *http.Request) {
	...
	sql := h.buildSQL(req.ScenarioID, req.Param, dataPath)
	if sql == "" {
...
func (h *InvestigateHandler) buildSQL(scenarioID, param, dataPath string) string {
	...
	switch scenarioID {
	case "iam-write-ops":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.recipientAccountId as account, r.sourceIPAddress, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventSource = 'iam.amazonaws.com' AND r.readOnly = 'false' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "iam-read-by-key":
		return fmt.Sprintf(`SELECT r.eventName, r.eventSource, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userIdentity.accessKeyId = '%s' AND r.eventSource = 'iam.amazonaws.com' ORDER BY r.eventTime DESC LIMIT 100;`, read, safeParam)

	case "iam-users-created":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as creator, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'CreateUser' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "iam-users-deleted":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'DeleteUser' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "access-denied-all":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as identity, r.eventName, r.eventSource, r.recipientAccountId as account, r.sourceIPAddress, COUNT(*) as count FROM (SELECT unnest(Records) as r FROM %s) WHERE r.errorCode IN ('AccessDenied', 'Client.UnauthorizedOperation') GROUP BY r.userIdentity.arn, r.eventName, r.eventSource, r.recipientAccountId, r.sourceIPAddress ORDER BY count DESC LIMIT 100;`, read)

	case "access-denied-by-identity":
		return fmt.Sprintf(`SELECT r.eventName, r.eventSource, r.errorCode, r.errorMessage, r.recipientAccountId as account, r.sourceIPAddress, r.eventTime FROM (SELECT unnest(Records) as r FROM %s) WHERE r.errorCode IN ('AccessDenied', 'Client.UnauthorizedOperation') AND r.userIdentity.arn = '%s' ORDER BY r.eventTime DESC LIMIT 100;`, read, safeParam)

	case "activity-by-ip":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as identity, r.eventName, r.eventSource, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.sourceIPAddress = '%s' ORDER BY r.eventTime DESC LIMIT 100;`, read, safeParam)

	case "ip-to-identity-map":
		return fmt.Sprintf(`SELECT r.sourceIPAddress as ip, r.userIdentity.arn as identity, COUNT(*) as call_count, MIN(r.eventTime) as first_seen, MAX(r.eventTime) as last_seen FROM (SELECT unnest(Records) as r FROM %s) WHERE r.sourceIPAddress IS NOT NULL AND r.sourceIPAddress NOT LIKE '%%.amazonaws.com' GROUP BY r.sourceIPAddress, r.userIdentity.arn ORDER BY call_count DESC LIMIT 100;`, read)

	case "ec2-instances-created":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as launcher, r.sourceIPAddress, r.recipientAccountId as account, r.awsRegion, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'RunInstances' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "describe-vpc-ec2-sg":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as identity, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('DescribeInstances', 'DescribeSecurityGroups', 'DescribeVpcs', 'DescribeSubnets', 'DescribeVolumes', 'DescribeNetworkInterfaces', 'DescribeAddresses') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "large-instances":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as launcher, r.sourceIPAddress, r.recipientAccountId as account, r.awsRegion, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'RunInstances' AND r.errorCode IS NULL ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "cross-account-all":
		return fmt.Sprintf(`SELECT r.userIdentity.accountId as source_account, r.recipientAccountId as target_account, r.userIdentity.arn as identity, r.eventName, r.eventSource, r.sourceIPAddress, r.eventTime FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userIdentity.accountId IS NOT NULL AND r.recipientAccountId IS NOT NULL AND r.userIdentity.accountId != r.recipientAccountId ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "cross-account-by-account":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as identity, r.eventName, r.eventSource, r.recipientAccountId as target_account, r.sourceIPAddress, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userIdentity.accountId = '%s' AND r.recipientAccountId IS NOT NULL AND r.recipientAccountId != '%s' ORDER BY r.eventTime DESC LIMIT 100;`, read, safeParam, safeParam)

	case "cross-account-role-assumptions":
		return fmt.Sprintf(`SELECT r.userIdentity.accountId as source_account, r.recipientAccountId as target_account, r.userIdentity.arn as caller, r.sourceIPAddress, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'AssumeRole' AND r.userIdentity.accountId IS NOT NULL AND r.recipientAccountId IS NOT NULL AND r.userIdentity.accountId != r.recipientAccountId ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "s3-data-access":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as identity, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventSource = 's3.amazonaws.com' AND r.eventName IN ('GetObject', 'PutObject', 'DeleteObject', 'CopyObject') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "secrets-accessed":
		return fmt.Sprintf(`SELECT r.eventName, r.eventSource, r.userIdentity.arn as identity, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('GetSecretValue', 'Decrypt', 'GetParameter', 'GetParameters', 'GetParametersByPath') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "activity-by-role":
		return fmt.Sprintf(`SELECT r.eventName, r.eventSource, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userIdentity.sessionContext.sessionIssuer.userName = '%s' ORDER BY r.eventTime DESC LIMIT 100;`, read, safeParam)

	case "role-across-accounts":
		return fmt.Sprintf(`SELECT r.userIdentity.sessionContext.sessionIssuer.userName as role_name, r.userIdentity.accountId as source_account, r.recipientAccountId as target_account, COUNT(*) as call_count FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userIdentity."type" = 'AssumedRole' AND r.userIdentity.accountId IS NOT NULL AND r.recipientAccountId IS NOT NULL AND r.userIdentity.accountId != r.recipientAccountId GROUP BY role_name, source_account, target_account ORDER BY call_count DESC LIMIT 50;`, read)

	case "console-logins":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as identity, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorMessage FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'ConsoleLogin' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "console-logins-failed":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as identity, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorMessage FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'ConsoleLogin' AND r.errorMessage IS NOT NULL ORDER BY r.eventTime DESC LIMIT 100;`, read)

	// ===== GuardDuty-Aligned Findings =====

	case "gd-credential-harvesting":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as identity, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('GetPasswordData', 'GetSecretValue', 'BatchGetSecretValue', 'GenerateDbAuthToken', 'GetAuthorizationToken', 'RequestCertificate') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-access-key-created-persistence":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as creator, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'CreateAccessKey' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-logging-disabled":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('StopLogging', 'DeleteTrail', 'UpdateTrail', 'DeleteFlowLogs', 'DisableAlarmActions', 'DeleteAlarms', 'PutEventSelectors', 'DeleteEventDataStore') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-password-policy-weakened":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('UpdateAccountPasswordPolicy', 'DeleteAccountPasswordPolicy') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-guardduty-disabled":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('DeleteDetector', 'DisableOrganizationAdminAccount', 'DeleteMembers', 'BatchDisableStandards', 'DisableSecurityHub', 'DeleteInsight') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-recon-enumeration":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as identity, COUNT(*) as api_calls, COUNT(DISTINCT r.eventName) as unique_apis, COUNT(DISTINCT r.eventSource) as services_probed, r.recipientAccountId as account FROM (SELECT unnest(Records) as r FROM %s) WHERE (r.eventName LIKE 'List%%' OR r.eventName LIKE 'Describe%%' OR r.eventName LIKE 'Get%%') AND r.userIdentity.invokedBy IS NULL GROUP BY r.userIdentity.arn, r.recipientAccountId HAVING COUNT(*) > 50 ORDER BY api_calls DESC LIMIT 50;`, read)

	case "gd-recon-by-identity":
		return fmt.Sprintf(`SELECT r.eventName, r.eventSource, r.recipientAccountId as account, r.sourceIPAddress, r.eventTime FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userIdentity.arn = '%s' AND (r.eventName LIKE 'List%%' OR r.eventName LIKE 'Describe%%' OR r.eventName LIKE 'Get%%') ORDER BY r.eventTime DESC LIMIT 100;`, read, safeParam)

	case "gd-snapshot-exfil":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('CreateSnapshot', 'CopySnapshot', 'ModifySnapshotAttribute', 'CreateDBSnapshot', 'CopyDBSnapshot', 'ModifyDBSnapshotAttribute', 'ShareImage', 'ModifyImageAttribute') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-s3-replication":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('PutBucketReplication', 'PutReplicationConfiguration', 'CopyObject') AND r.eventSource = 's3.amazonaws.com' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-destructive-actions":
		return fmt.Sprintf(`SELECT r.eventName, r.eventSource, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('TerminateInstances', 'DeleteDBInstance', 'DeleteDBCluster', 'DeleteBucket', 'DeleteStack', 'DeleteUser', 'DeleteRole', 'DeletePolicy', 'DeleteSecurityGroup', 'DeleteSubnet', 'DeleteVpc', 'DeleteFunction20150331') AND r.errorCode IS NULL ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-s3-public-access":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('PutBucketPolicy', 'PutBucketAcl', 'PutObjectAcl', 'DeletePublicAccessBlock', 'PutPublicAccessBlock', 'PutBucketPublicAccessBlock') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-persistence-mechanisms":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('CreateAccessKey', 'ImportKeyPair', 'CreateLoginProfile', 'UpdateLoginProfile', 'CreateUser', 'CreateRole', 'CreateInstanceProfile') AND r.errorCode IS NULL ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-network-persistence":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('AuthorizeSecurityGroupIngress', 'CreateVpcPeeringConnection', 'AcceptVpcPeeringConnection', 'CreateNetworkAclEntry', 'ReplaceNetworkAclEntry', 'CreateRoute', 'AttachInternetGateway') AND r.errorCode IS NULL ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-privesc-iam":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('PutUserPolicy', 'PutRolePolicy', 'PutGroupPolicy', 'AttachUserPolicy', 'AttachRolePolicy', 'AttachGroupPolicy', 'AddUserToGroup', 'CreatePolicyVersion', 'SetDefaultPolicyVersion') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-privesc-assume-role":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as caller, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('AssumeRole', 'AssumeRoleWithSAML', 'AssumeRoleWithWebIdentity') AND r.userIdentity.invokedBy IS NULL ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-root-usage":
		return fmt.Sprintf(`SELECT r.eventName, r.eventSource, r.sourceIPAddress, r.recipientAccountId as account, r.userAgent, r.eventTime, r.errorCode FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userIdentity."type" = 'Root' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-s3-block-public-disabled":
		return fmt.Sprintf(`SELECT r.eventName, r.userIdentity.arn as actor, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName IN ('DeletePublicAccessBlock', 'PutPublicAccessBlock') ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-console-multi-geo":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as identity, COUNT(DISTINCT r.sourceIPAddress) as unique_ips, COUNT(*) as login_count FROM (SELECT unnest(Records) as r FROM %s) WHERE r.eventName = 'ConsoleLogin' GROUP BY r.userIdentity.arn HAVING COUNT(DISTINCT r.sourceIPAddress) > 1 ORDER BY unique_ips DESC LIMIT 50;`, read)

	case "gd-instance-cred-exfil":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as role_arn, r.userIdentity.sessionContext.sessionIssuer.userName as role_name, r.sourceIPAddress, r.eventName, r.recipientAccountId as account, r.eventTime FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userIdentity."type" = 'AssumedRole' AND r.userIdentity.invokedBy IS NULL AND r.sourceIPAddress NOT LIKE '10.%%' AND r.sourceIPAddress NOT LIKE '172.%%' AND r.sourceIPAddress NOT LIKE '192.168.%%' AND r.sourceIPAddress NOT LIKE '%%.amazonaws.com' ORDER BY r.eventTime DESC LIMIT 100;`, read)

	case "gd-pentest-tools":
		return fmt.Sprintf(`SELECT r.userIdentity.arn as identity, r.userAgent, r.eventName, r.sourceIPAddress, r.recipientAccountId as account, r.eventTime FROM (SELECT unnest(Records) as r FROM %s) WHERE r.userAgent LIKE '%%kali%%' OR r.userAgent LIKE '%%Kali%%' OR r.userAgent LIKE '%%parrot%%' OR r.userAgent LIKE '%%Parrot%%' OR r.userAgent LIKE '%%pentoo%%' OR r.userAgent LIKE '%%Pentoo%%' OR r.userAgent LIKE '%%pacu%%' OR r.userAgent LIKE '%%Pacu%%' OR r.userAgent LIKE '%%prowler%%' OR r.userAgent LIKE '%%ScoutSuite%%' ORDER BY r.eventTime DESC LIMIT 100;`, read)
